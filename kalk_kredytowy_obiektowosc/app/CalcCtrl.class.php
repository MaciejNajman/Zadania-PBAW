 ...
 * @author Przemys³aw Kud³acik in collaboration with Maciej Najman
 *
 */
class CalcCtrl {
	...
	public function validate() {
		...
		if (! (isset ( $this->form->kwota ) && isset ( $this->form->ile_lat ) && isset ( $this->form->opr ))) {
			...
			return false; //zakoñcz walidacjê z b³êdem
		} else { 
			$this->hide_intro = true; //przysz³y pola formularza, wiêc - schowaj wstêp
		}
		...
		if ($this->form->ile_lat == "") {
			$this->msgs->addError('Nie podano czasu splaty');
		}
		if ($this->form->opr == "") {
	...
	public function generateView(){
		...
		$smarty->assign('page_header','Kalkulator w PHP');
